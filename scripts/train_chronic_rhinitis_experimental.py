#!/usr/bin/env python3
"""
Experimental training script for Chronic Rhinitis (Chronic Upper Respiratory Disease)
Research Goal: Identify environmental exposure lag periods for chronic respiratory illness
"""
...
def setup_logging(config):
    ...
    log_file = log_dir / f"train_chronic_rhinitis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def optimize_hyperparameters(X_train, y_train, selected_features, config, logger):
    ...
    study = optuna.create_study(direction='maximize', study_name='chronic_rhinitis_optimization')
...
def save_results(model, selected_features, best_params, y_test, test_pred, config, logger):
    ...
    # Save model
    model_file = results_dir / f"model_chronic_rhinitis_{timestamp}.pkl"
    with open(model_file, 'wb') as f:
        pickle.dump(model, f)
    logger.info(f"Model saved: {model_file}")
    
    # Save selected features
    features_file = results_dir / f"selected_features_chronic_rhinitis_{timestamp}.csv"
    ...
    importance_file = results_dir / f"feature_importance_chronic_rhinitis_{timestamp}.csv"
    importance_df.to_csv(importance_file, index=False)
    logger.info(f"Feature importance saved: {importance_file}")
    
    # Save test predictions
    predictions_file = results_dir / f"test_predictions_chronic_rhinitis_{timestamp}.csv"
    pd.DataFrame({
    ...
    metadata_file = results_dir / f"metadata_chronic_rhinitis_{timestamp}.yaml"
...
def main():
    # Load configuration
    config_path = project_root / "configs" / "config_chronic_rhinitis_experimental.yaml"
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)
    
    logger = setup_logging(config)
    logger.info("=" * 80)
    logger.info("EXPERIMENTAL TRAINING - CHRONIC RHINITIS (CHRONIC RESPIRATORY)")
    logger.info("=" * 80)
    ...
    if not data_file.exists():
        ...
        logger.error("Please run preprocessing first: python scripts/preprocess_chronic_rhinitis_experimental.py")
        return
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("✓ CHRONIC RHINITIS TRAINING COMPLETE")
    logger.info("=" * 80)
