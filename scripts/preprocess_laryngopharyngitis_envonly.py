#!/usr/bin/env python3
"""
Environmental-Only preprocessing for Acute Laryngopharyngitis
...
def setup_logging(config):
    ...
    log_file = log_dir / f"preprocess_laryngopharyngitis_envonly_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    # Load configuration
    config_path = project_root / "configs" / "config_laryngopharyngitis_envonly.yaml"
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)
    
    logger = setup_logging(config)
    logger.info("=" * 80)
    logger.info("ENVIRONMENTAL-ONLY PREPROCESSING - ACUTE LARYNGOPHARYNGITIS")
    logger.info("=" * 80)
