...
class KMAClimateAnalyzer:
    ...
    def handle_missing_values_after_grouping(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in continuous_cols:
            if col in df.columns:
                ...
                df[col] = df.groupby('RegionCode')[col].transform(
    ...
    def export_results(self, df: pd.DataFrame, analysis_results: Dict[str, pd.DataFrame], 
        ...
        try:
            ...
            df.to_csv(f'{output_dir}/2018_processed_climate_data.csv', encoding='utf-8', index=False)
...
def main():
    # Set the file path
    file_path = os.path.join(os.path.expanduser("~"), "Desktop", "Illness Prediction", "Raw Data", "Climate Data 2", "2018.csv")
