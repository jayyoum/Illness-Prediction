...
output_base_dir = "/Users/jay/Desktop/Illness Prediction/Processed Data/Regression Results/RF with Split/FS_lag3"
...
for filename in os.listdir(merged_data_dir):
    ...
    for region in df["RegionCode"].unique():
        ...
        if len(region_df) < 20:  # Need enough to split
            continue
    ...
    output_path = os.path.join(output_dir, f"RF_split_{lag_label}.csv")
    pd.DataFrame(results).to_csv(output_path, index=False)
    print(f"✅ RF (with split) done for {illness_name} — saved to {output_path}")
