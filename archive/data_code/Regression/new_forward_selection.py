...
output_base_dir = "/Users/jay/Desktop/Illness Prediction/Processed Data/Regression Results/Selected Variables/FS_grouped"
lags = range(0, 8)
excluded_variable = "MaxWindDir"  # ← REMOVE THIS VARIABLE BEFORE FS
...
for illness_name in grouped_illnesses:
    for lag_days in lags:
        ...
        excluded_cols = {"RegionCode", "Month", "CaseCount", "ParsedDateTime", "IllnessName", "DateTime", excluded_variable}
        ...
        if not env_features:
            print(f"⚠️ No usable environmental variables for {illness_name} (lag {lag_days})")
            continue
        ...
        print(f"\n📊 Forward Selection | {illness_name} | Lag {lag_days}")
        selected_vars = forward_selection(X, y, verbose=False)
        ...
        output_path = os.path.join(illness_dir, f"forward_selection_lag{lag_days}.csv")
